...
data$Vb <- solve(diag(10000, 4))  # regression b precisions, dimension = 4
data$s1 <- 0.1  # error prior n/2
data$s2 <- 0.1  # error prior SS/2
```
...
inits <- list()
for(i in 1:nchain){
  inits[[i]] <- list(b = rnorm(4, 0, 5),  # dimension = 4 (b0, b1, b2, b3)
...
# sample
jags.out <- coda.samples(model = j.model,
                         variable.names = c("b", "S"),  # var. names in the inits
                         n.iter = 5000)
```
...
```{r}
# convergence - visual assessment
plot(jags.out)  # overlapping & randomly bouncing -> looks converged

# convergence - Brooks-Gelman-Rubin (BGR) statistics
gelman.diag(jags.out)  # less than 1.01 -> excellent!
```

```{r}
# define the "burn in" period
BGR <- gelman.plot(jags.out)  # BGR > 1.05 = burn-in period
```
...
```{r}
# compare with frequentist method (i.e. classic linear regression)
summary(lm(y ~ x1 * x2, data = data.frame(x1 = data[[1]],
                                          x2 = data[[2]],
                                          y = data[[3]])
           )
        )

# Almost the same results (mean & uncertainty of each parameter estimate)
